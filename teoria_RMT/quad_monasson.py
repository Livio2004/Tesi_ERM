...
for z_real in lambda_vals:
    ...
    for _ in range(300):
        ...
        s_next = 1.0 / (sum_term - z)
        s = (1.0 - alpha) * s + alpha * s_next
    
    #utilizziamo la s come una bnuova guess
    s_guess = s
    rho_analitica.append(np.abs(np.imag(s)) / np.pi)
# plot
...
plt.axvline(gamma_hat_0, color='orange', linestyle='-.', label=r"$\hat{\Gamma}(0)$ (Autovalore estensivo)")
plt.yscale('log')
